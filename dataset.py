...
class HandCropDataset(Dataset): 
    ...
    def get_full_sample(self, item):
        ...
        real_hand = None if self.image_dataset.is_test else \
                    self.transform(real_img[top: top + self.crop_size, left: left + self.crop_size,  :])
        fake_hand = self.transform(fake_img[top: top + self.crop_size, left: left + self.crop_size,  :])
        ...
        # keep full fake image to visualize enhancement result
        return real_hand, fake_hand, \
               top, top + self.crop_size, \
               left, left + self.crop_size, \
               real_img, fake_img

    def __getitem__(self, item):
        real_hand, fake_hand, _, _, _, _, _, _ = self.get_full_sample(item)
        return {'real_hands': real_hand, 'fake_hands': fake_hand}
