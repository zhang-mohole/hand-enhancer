...
class Trainer(object):
    ...
    def train_generator(self, g, d, g_opt):
        ...
        real_hands = batch['real_hands']
        fake_hands = batch['fake_hands']

        g_opt.zero_grad()

        residuals = g(fake_hands)
        enhanced_heads = fake_hands + residuals
        fake_features = d.extract_features(enhanced_heads)
        with torch.no_grad():
            real_features = d.extract_features(real_hands)
        gen_loss = self.gen_loss(fake_features, real_features)

        recon_loss = self.recon_loss(enhanced_heads, real_hands)
    ...
    def train_discriminator(self, g, d, d_opt):
        ...
        residual = g(fake_hands)
        enhanced_heads = fake_hands + residual
    ...
    def validate_and_save(self, generator, batch_num):
        ...
        real_hand, fake_hand, top, bottom, left, right, real_full, fake_full \
            = self.dataset.get_full_sample(idx)

        with torch.no_grad():
            fake_hand.unsqueeze_(0)
            real_hand = real_hand.to(self.device)
            fake_hand = fake_hand.to(self.device)
            residual = generator(fake_hand)
            enhanced = fake_hand + residual

        fake_hand.squeeze_()
        enhanced.squeeze_()
        residual.squeeze_()
        image = torch.cat((real_hand, fake_hand, residual, enhanced), dim=2)
